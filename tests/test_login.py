...
def test_login(session, apis, case_data):
    """
    通过参数化测试测试登录接口
    """
    ...
    login_security = re.findall(
        r'"login_security" value="(.*?)" />', session.request(api.method, api.url).text
    )
    ...
    req = session.request(api.method, api.url, data=api.data, allow_redirects=False)
